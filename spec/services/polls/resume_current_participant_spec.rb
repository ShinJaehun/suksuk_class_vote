...
RSpec.describe Polls::ResumeCurrentParticipant do
  describe "#call" do
    it "sets current participant to the first unprocessed poll participant" do
      ...
      expect(poll.poll_events.last.details).to include("to_poll_participant_id" => participants[1].id)
    end
    ...
    it "fails when there is no unprocessed poll participant" do
      ...
      expect(poll.poll_progress.reload.current_poll_participant).to be_nil
    end

    it "does not change participation, poll_option tallies, or poll status" do
      poll = create_in_progress_poll
      participants = poll.poll_participants.order(:number)
      create(:poll_participation, poll_participant: participants[0], status: :completed)
      poll_option_tally = poll.poll_option_tallies.first
      poll_option_tally.update!(votes_count: 1)
      poll.poll_progress.update!(current_poll_participant: nil)

      expect do
        described_class.new(poll: poll).call
