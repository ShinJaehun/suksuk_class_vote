...
RSpec.describe Elections::LockBallot do
  describe "#call" do
    it "locks an open ballot for the current voter" do
      election_session = started_session_with_open_ballot
      ...
      event = election_session.election_events.where(event_type: :ballot_locked).sole
    ...
    it "fails without an actor" do
      ...
      expect_no_lock_change(election_session)
    end
    ...
    it "fails without progress" do
      ...
      expect(election_session.election_events.where(event_type: :ballot_locked)).to be_empty
    end
    ...
    it "fails when already locked" do
      election_session = started_session_with_open_ballot
      ...
      expect(result).not_to be_success
      expect(result.error_message).to include("이미 ballot이 잠겨 있습니다.")
      expect(election_session.election_progress.reload).to be_locked
      expect(election_session.election_events.where(event_type: :ballot_locked)).to be_empty
    end
  end

  def started_session_with_open_ballot
    ...
    Elections::OpenBallot.new(election_session: election_session, actor: teacher).call

    election_session.reload
  end

  def expect_no_lock_change(election_session)
