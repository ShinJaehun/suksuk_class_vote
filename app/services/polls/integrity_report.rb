  ...
  class IntegrityReport
    ...
    def add_poll_progress_issues(report_issues)
      if poll.in_progress?
        ...
        report_issues << issue("진행 중인 투표의 현재 투표자를 찾을 수 없습니다.") if current_poll_participant.blank?
        if current_poll_participant.present? && current_poll_participant.poll_id != poll.id
          report_issues << issue("현재 투표자가 이 투표의 투표자 명단에 속하지 않습니다.")
        end
      elsif poll.closed?
    ...
    def add_participation_issues(report_issues)
      ...
      if poll.closed? && unprocessed_count.positive?
        report_issues << issue("종료된 투표에 미처리 투표자가 남아 있습니다.")
      end
    end
    ...
    def current_participant_missing_only_issue?
      issues.map(&:message) == ["진행 중인 투표의 현재 투표자를 찾을 수 없습니다."]
    end
