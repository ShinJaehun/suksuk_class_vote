module Polls
  class MarkNextSessionParticipantAbsent
    ...
    def call
      ...
      ActiveRecord::Base.transaction do
        ...
        if errors.empty?
          next_participant.lock!
          next_participant.create_poll_participation!(
            status: :absent,
            recorded_at: Time.current
          )
          record_event(next_participant)
        end
    ...
    def validate_locked_state(progress, current_participant)
      ...
      errors << "투표 화면이 잠긴 상태에서만 미참여 처리할 수 있습니다." unless progress&.ballot_locked?
