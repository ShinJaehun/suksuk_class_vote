...
학급 `Poll` 선거에는 후보자 사진 기능을 도입하지 않는다.
후보자 사진은 별도 Admin `Election` 흐름의 `ElectionCandidate`에만 적용하며,
전교어린이회임원선거 등에서 후보 식별을 돕기 위한 선택 입력이다.
사진은 결과 집계, 검산, 운영 기록에 필요한 정보가 아니다.
