...
| 리소스/액션 | admin |  teacher | guest | 비고                         |
| ------ | ----: | -------: | ----: | -------------------------- |
| 목록 조회  | 전체 가능 | 본인 것만 가능 |    불가 | `ElectionPolicy::Scope` 구현 |
| 상세 조회  | 전체 가능 | 본인 것만 가능 |    불가 | `show` 구현                  |
...
* 원본 `VoterGroup` 변경은 이미 시작된 선거의 snapshot 명단에 영향을 주지 않는다.
* 선거에 연결된 snapshot 명단은 수정하지 않는다.
* 예외가 필요하다면 admin 권한, 확인 절차, 감사 로그를 함께 검토한다.
...
`PollingStation`은 현재 투표 위치를 복구하기 위한 진행 상태 컨테이너이며, 후보 선택 결과나 득표수를 저장하지 않는다.
teacher는 본인 선거의 `PollingStation`만 진행할 수 있고, admin은 전체 `PollingStation`에 접근할 수 있다.
