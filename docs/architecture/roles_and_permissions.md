...
| ---------- | ----: | -----------: | ----: | ----------------------------- |
| 교사 계정 목록   |    가능 |           불가 |    불가 | admin 전용                      |
| 교사 계정 생성   |    가능 |           불가 |    불가 | 초기에는 admin 생성 또는 교사 회원가입 중 택일 |
| 교사 계정 수정   |    가능 | 본인 기본 정보만 가능 |    불가 | 상세 범위는 구현 시 결정                |
| 교사 계정 비활성화 |    가능 |           불가 |    불가 | 감사 로그 고려                      |
...
| 리소스/액션            | admin |   teacher | guest | 비고                  |
| ----------------- | ----: | --------: | ----: | ------------------- |
| 목록 조회             | 전체 가능 |  본인 것만 가능 |    불가 | policy scope 필요     |
| 상세 조회             | 전체 가능 |  본인 것만 가능 |    불가 |                     |
| 생성                |    가능 |        가능 |    불가 | teacher는 본인 소유로 생성  |
| 수정                |    가능 |  본인 것만 가능 |    불가 | 선거 시작 후 제한 필요       |
| 삭제                |    가능 |  본인 것만 가능 |    불가 | 이미 선거에 사용된 경우 제한 필요 |
| 학생 명단 bulk import |    가능 | 본인 그룹에 가능 |    불가 | preview 후 commit    |
