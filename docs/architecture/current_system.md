...
- 선거 시작 후 후보자 추가/수정/삭제 차단
- 진행 중인 선거가 사용하는 투표자 그룹/학생 명단 변경 차단
- 선거 종료 후에는 `ElectionVoter` snapshot 기준으로 선거 자료를 보존하고 원본 투표자 그룹/학생 명단 편집/추가/삭제 허용
- 현재 투표자를 미참여 또는 기권으로 확정 처리하는 기능 추가
...
- `Elections::IntegrityReport` service와 선거 상세 상태 점검 카드 추가
- in_progress / closed 상태의 운영 요약 표시 추가
- 현재 투표자 포인터가 비어 있는 제한 상황에서 첫 미처리 학생으로 재개하는 기능 추가
- `ElectionEvent` 모델과 선거 운영 이벤트 기록 추가
- 선거 상세 화면에 최근 운영 기록 표시 추가
- 문서 기반 설계 정리 중
