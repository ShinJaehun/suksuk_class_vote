...
schema도 제거 완료됐으며, 과거 migration과 Election ID 6 복원 계약은 유지한다.
...
- PostgreSQL 사용
- Tailwind CSS 사용
- 투표 도메인은 Classroom/PollSession runtime으로 전환 완료
...
- `/teachers`는 admin과 manager가 이름·로그인 ID·학년·담당 반을 관리하는 canonical 화면이다. 일반 teacher는 자기 기본정보 수정 UI 없이 현재 비밀번호 확인 후 비밀번호만 직접 변경할 수 있다.
...
- `/schools/:id` 상단은 현재 active 선생님·active Classroom·active Classroom의 active Student 수와 SchoolMembership manager를 보여주는 학교 운영 overview다. `/schools/:id/edit`에서 admin은 학교 이름과 같은 학교의 active 선생님 중 대표 선생님을 변경하며, 기존 manager 교체는 grade·담당 Classroom·계정 상태를 건드리지 않고 원자적으로 처리한다.
- School은 `active` 상태를 가지며 admin만 활성화·비활성화할 수 있다. 비활성 School은 read-only 운영 동결 상태로, Teacher·Classroom·Student·Poll/history와 배정을 변경하지 않으면서 manager와 teacher의 기존 현황·명단·투표 결과 조회는 유지하고 학교 운영 write를 막는다. admin은 복구·정리를 위한 School·Teacher·Classroom·Student 관리를 계속할 수 있지만, admin을 포함한 모든 사용자의 새 투표 생성과 투표 runtime operation은 School 재활성화 전까지 허용하지 않는다. 진행 중인 전교투표가 있으면 먼저 종료하거나 중단하도록 School 비활성화를 거부하며 어떤 학급투표도 변경하지 않는다. 전교투표가 진행 중이 아니면 비활성화 과정에서 해당 학교의 진행 중 학급투표만 기존 중단 lifecycle로 중단하고 기록·결과를 보존한 뒤 School의 `active`를 변경한다. 정상 종료로 처리하거나 그 밖의 child 상태를 변경하지 않으며, 재활성화는 School의 `active`만 복원한다. 삭제는 membership과 Classroom이 없고 다른 reference가 destroy를 허용하는 사용되지 않은 비활성 School만 가능하다. 사용 이력이 있으면 비활성 상태로 보존하며 학교 초기화 기능은 제공하지 않는다. 학년도별 과거 운영 조회와 rollover는 별도 historical lifecycle 작업으로 분리한다.
...
- 대표 선생님은 자기 비밀번호만 변경할 수 있고 자기 계정을 비활성화하거나 삭제할 수 없다. 자기 학교 일반 선생님과 global admin의 기존 관리 범위는 유지되며, 자기 lifecycle 제한은 서버의 policy에서 강제한다.
...
- legacy DB table 제거 완료; 과거 migration과 Election ID 6 복원 계약은 유지
...
Election과 ParticipantGroup/ParticipantSlot runtime 및 관련 DB table·column은 제거 완료됐다.
...
기존 Election ID 6은 추후 검증된 backup을 기준으로 historical Poll로 변환한다.
