...
- School은 `active` 상태를 가지며 admin만 활성화·비활성화할 수 있다. 비활성화는 Teacher·Classroom·Student·Poll/history와 배정을 변경하지 않고 일반 학교 운영 접근만 중단하며, admin은 계속 조회·설정할 수 있다. 삭제는 membership과 Classroom이 없고 다른 reference가 destroy를 허용하는 사용되지 않은 비활성 School만 가능하다. 사용 이력이 있으면 비활성 상태로 보존하며 학교 초기화 기능은 제공하지 않는다. 학년도별 과거 운영 조회와 rollover는 별도 historical lifecycle 작업으로 분리한다.
